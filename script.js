...
function applyCssVars(settings) {
    ...
    root.style.setProperty('--search-opacity', `${settings.searchOpacityPercent}%`);
    ...
    await withStore('readwrite', store => {
        return new Promise((resolve, reject) => {
            ...
            for (const file of fileList) {
                ...
                req.onsuccess = () => {
                    completed++;
                    if (completed === fileList.length && !hasError) {
                        resolve();
                    }
                };
                req.onerror = () => {
...
function setupSettingsUi(controllers) {
    ...
    async function renderPreviewGrid() {
        ...
        if (currentSettings.imageSource === 'imported') {
            ...
            try {
                const packagedList = await loadPackagedImageList();
                items = packagedList.map(name => ({ name, type: 'packaged' }));
            } catch (_) {
                items = [];
            }
        }
        ...
        for (const item of items) {
            ...
            // 设置背景图 - 对两种类型都使用 blob URL 方式
            try {
                let url = null;
                if (item.type === 'packaged') {
                    // packaged: 通过 fetch 获取并创建 blob URL
    ...
    if (btnColorPicker) {
        btnColorPicker.addEventListener('input', () => {
            applyCssVars(normalizeSettings({ ...currentSettings, btnColorHex: btnColorPicker.value }));
        });
        btnColorPicker.addEventListener('change', async () => {
            await save({ btnColorHex: btnColorPicker.value });
        });
    }
