...
- **v1.4.1 (Current)**: Fixed the "Search Box & Button Spacing (px)" setting — previously adjusting the spacing also moved the search box itself because the search container was vertically centered; it now changes only the gap between the search box and the button, leaving the search box position untouched. The search button now sits on the right of the search box, vertically centered, with a glassmorphism style. Current parameters locked in as defaults.
